...
값의 평균을 구해주는 함수, 반환값은 `Double` 이다. 끝에 `Int` 가 붙은 이유는 `Int` 를 데이터를 맵핑하기 때문이다.
...
그룹핑은 매개변수로 받은 값을 기준으로 맵 자료형으로 반환해준다.
```java
Map<Integer, List<Product>> collectorMapOfLists = productList.stream()
  .collect(Collectors.groupingBy(Product::getAmount));

/**
 *  amount 를 기준으로 값을 그룹핑(같은 리스트로 묶어서) 맵형식으로 반환해준다.
 *  23, 13, 14 값을 키값으로 객체를 분류해주었다.
 */
{
...
/**
 *  Amount 가 15 를 넘으면 true, 못넘으면 false 로 분류해준다.
 */
{
...
```java
/**
 * 먼저 리스트 자료형을 Set 으로 바꾸고, 수정불가능한 Set 으로 변경해주었다.
 */
...
## Iterating
컬렉션 내부를 순회하면서 `forEach()` 의 매개변수로 받은 함수를 실행해 준다.
```java
