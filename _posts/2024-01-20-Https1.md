...
응답을 받는때까지 해외를 2번이나 갔다오다보니 매우 느리다. 실제로 적용해봤더니 매우 느리다. 10배 정도 느림을 체감할 수 있다. 페이지가 로딩되는데 1초정도 걸린것 같았다. Cloudflare 를 사용하지 않는다면 
...
가변을 사용하면 겉으로는 https 를 사용하기때문에 당장 급하고, 복잡하게 설정할 필요없다면 사용해도 된다. 하지만 보안성은 취약하기때문에 왠만하면 전체로 설정해주자. 전체와 전체(엄격) 의 차이는 Cloudflare 에서 제공하는 인증서를 사용하는지 여부다. 
...
docker-compose 사용할 때 nginx.conf에서 'directive is not allowed' 에러가 발생한다. 항상 첫라인에서 안된다고 에러가 난다. nginx.conf 에서 첫라인에 worker_process 를 사용하면 위와 같이 worker_process 가 허용되지 않았다고 에러가 나고, 이 글에서 설정한대로 하면 http 가 허용되지 않았다고 에러가 난다. 

기존의 docker-compose 에서는 아래와 같이 폴더를 옮기는 방식이었는데
